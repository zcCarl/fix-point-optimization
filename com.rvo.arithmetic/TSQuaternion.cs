...
namespace RVO.Arithmetic
{
    ...
    public struct TSQuaternion
    {
        ...
        {
            FP normSqr = rotation.x * rotation.x + rotation.y * rotation.y + rotation.z * rotation.z + rotation.w * rotation.w;
            if (normSqr == FP.Zero)
            {
                return TSQuaternion.identity;
            }
            FP invNorm = FP.One / normSqr;
            TSQuaternion conjugate = TSQuaternion.Conjugate(rotation);
